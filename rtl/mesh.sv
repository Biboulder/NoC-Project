...
module mesh #(
    parameter int SIDE = 3 //max 3 for now as we have 12 bits for header so max 4 hops
) (
    input logic clk,
    input logic rst_n,

    input logic valid_in_l [SIDE*SIDE],
    input packet_t pkt_in_l [SIDE*SIDE],
    output logic valid_out_l [SIDE*SIDE],
    output packet_t pkt_out_l [SIDE*SIDE]
);
     ...
     generate
        for (genvar r = 0; r < SIDE; r++) begin : row_gen
            for (genvar c = 0; c < SIDE; c++) begin : col_gen
                ...
                // Resolve each input to a plain wire before
                // instantiation (avoids ternaries inline in the port
                // list, which trips an iverilog elaboration bug).
